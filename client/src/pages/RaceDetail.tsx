...
function extractEmailFromDescription(description?: string): string {
  const text = String(description || "");
  const match = text.match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i);
  return (match?.[0] || "").trim().toLowerCase();
}

function resolveCreatorEmail(race: Race | undefined, subscriberEmail: string): string {
  if (!race) return "";
  if (race.createdByEmail) return race.createdByEmail.trim().toLowerCase();

  const emailInDescription = extractEmailFromDescription(race.description);
  if (emailInDescription) return emailInDescription;

  const generatedByStudio = /generated\s+from\s+subscriber\s+studio/i.test(String(race.description || ""));
  if (generatedByStudio && subscriberEmail) return subscriberEmail;
    ...
    queryFn: async () => {
      ...
      try {
        const response = await apiRequest<SubscriberProfile>(
          "GET",
          `/api/subscriber-profiles?email=${encodeURIComponent(creatorEmail)}`,
        );
              ...
              <div>
                ...
                <div className="flex items-center gap-4 flex-wrap">
                  ...
                  <div className="flex items-center gap-1 text-sm text-muted-foreground">
                    ...
                    <span>
                      Created {formatCreatedDate(raceCreatedAt)}
                    </span>
                  </div>
                  {lastCheckedAt && (
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                {subscriberProfile ? (
                  <div>
                    ...
                    <Link href={`/subscriber-profile/${encodeURIComponent(creatorEmail)}`}>
                  ...
                  <div>
                    <span className="text-muted-foreground">Generated by </span>
                    <Link href={`/subscriber-profile/${encodeURIComponent(creatorEmail)}`}>
                      <a className="font-medium text-primary hover:underline">{displayNameFromEmail(creatorEmail)}</a>
                    </Link>
                    <span className="text-muted-foreground"> in Subscriber Studio</span>
                  </div>
                )}
